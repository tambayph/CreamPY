...
# -*- coding: utf-8 -*-
"""
Created on Wed Jan 10 16:26:20 2024
...
dts = 'e1'
# indir = f'/home/nms/ecmwf/ens/{dts}/{init}'
...
for fhour in fhours:
    ...
    ### Plot title text
    ax.set_title(f'ECMWF IFS ENS 0.1deg\n{i}-Hour Mean Accumulated Rainfall (mm)', loc='left', fontsize=12, pad=8)
    ...
    ### Save plot as image
    plt.savefig(f'{outdir}/ecmwf_ens_mean_pcp_{init_date:%Y%m%d%H}_f{fhour:03d}_{init_date+timedelta(hours=fhour-i):%Y%m%d%H}_to_{init_date+timedelta(hours=fhour):%Y%m%d%H}', dpi=100, bbox_inches='tight')
