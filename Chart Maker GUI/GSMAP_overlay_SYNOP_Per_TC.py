...
with open("inputb.txt", "r") as file:
    ...
    selected_file_pathb = linesb[0].strip()
    selected_file_dirc = linesb[1].strip()
    selected_file_pathc = linesb[2].strip()
    selected_file_dirb = linesb[3].strip()
    
# Filename of the chart
save_name = selected_file_dirb

# OPEN THE FILES

# TC best track
track_fname = selected_file_pathb

# Path of the directory of GSMAP files
path = selected_file_dirc

# Synop and Agromet RR observations
rr_obs = pd.read_csv(selected_file_pathc)
########################################################################################################################
...
rgb = [[255,255,255],[127,255,0],[0,139,2],[255,255,0],[255,127,0],[255,0,0],[255,0,255],[139,0,139]]
rgb=np.array(rgb)/255.
cmap = ListedColormap(rgb,"")
boundaries = [0,25, 50, 100, 200, 300, 400, 500, 5000]
...
cbar = fig.colorbar(cs, pad=0, orientation="horizontal",
                    ticks=[0,25, 50, 100, 200, 300, 400, 500, 5000], cax=cbaxes)
cbar.ax.set_xticklabels([" ","25","50","100","200","300", "400", "500"," "], fontsize=7)
